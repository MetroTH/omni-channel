...
export async function POST(request: NextRequest) {
  ...
  if (!supabaseUrl || !serviceRoleKey) return NextResponse.json({ error: "Server misconfiguration: missing service role key" }, { status: 503 });

  const adminAuthClient = createSupabaseAdminDirectClient<Database>(supabaseUrl, serviceRoleKey, { auth: { autoRefreshToken: false, persistSession: false } });
  ...
  return NextResponse.json({ success: true, userId: inviteData.user?.id ?? null });
}
